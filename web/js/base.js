...
window.addEventListener('message', event => { if (vscode_CBK(event.data)) return })
...
function vscode_Eval(req = {})
{
	...
	const ret = (v) => {
		const f = vscode_Request.bind(null, req);			return req.call ? f() : f
	}
...
function parse_ExeIcon(x, prefix = 'exeicon_')
{
	if (!x) return prefix

	const i = $('#' + prefix + x.id);				if (x.icon) return i.removeClass('icon-none')
	
	return setTimeout(async () => {
		const r = await vscode_Eval().root.exeIcon(x.name)
		if (r)	{ i.attr('src', URI.parse(r));   i.removeClass('icon-none') }
		i.on("error", function () { this.onerror = null;    i.attr("src", "");    i.addClass('icon-none icon-err') })
	})
}
