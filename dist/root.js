...
{
    return !exports.debug && !force
    ?
    { begin(){return this}, end(){}, start(){}, stop(){} }
    :
    {
        ...
        stop(v)  { let t1  = performance.now();    console.log(this.N, v, (t1 - this.t0).toFixed(1) + "ms") }
    }
}
...
{
    if (typeof fName === 'object') fName = fName.fileName
...
function readDir(value)
{
    ...
    try {    result = fs.readdirSync(value)    }catch{}
...
{
    fName = typeof fName === 'object' ? fName.uri.fsPath : fName
    
    if (ext) fName = extFile(fName, ext)
    
    try {    return fs.readFileSync(fName, 'utf-8')    } catch {    return def    }
}
...
function writeFile(fName, txt = '', ext)
{
    ...
    try {    fs.writeFileSync(fName, txt, 'utf-8');    return true    } catch {    return false    }
}
...
{
    const name = path.dirname(fName) + path.sep + path.parse(fName).name
    
    if (!Array.isArray(ext))  return name + ext
...
{
    try {
        const v = fs.readFileSync(fName, 'utf-8').replace(/\/\*[\s\S]*?\*\//g, '');      return JSON.parse(v)
    }
...
{
    try {
        const v = JSON.stringify(value, null, '\t');      fs.writeFileSync(fName, v, 'utf-8');      return true
    }
    catch (e) {  if (fnErr) fnErr(e)  }
}
...
function openURL(url)
{
    ...
    return child.exec(exports.win ? `start "" "${url}"` : `xdg-open '${url}'`)
}
...
{
    try {
        vscode.workspace.openTextDocument(vscode.Uri.file(url)).then(doc => vscode.window.showTextDocument(doc, prm))
    }catch{}
}
...
function winver(fName)
{
    ...
    tmp = tmp.match(/\0StringFileInfo\0([\s\S]+?)\0VarFileInfo\0/i)
    tmp = tmp ? tmp[1].replace(/[\x00-\x1F]/g, '\0') : ''
...
{
    const rxp = new RegExp(mask, 'i'),   dir = dirPath.replace(/[\/\\]$/,'')
...
function searchDirs(dirPath, mask = '', deep = true)
{
    ...
    const rxpFile = new RegExp(mask[0], 'i')
    const rxpDir  = new RegExp(mask[1], 'i')
    
    const dir = dirPath.replace(/[\/\\]$/,''),   files = [],   dirs = []
	...
	for (const file of readDir(dir))
    {
		...
		if (fs.statSync(value).isDirectory())   { if (deep >= 0 && rxpDir.test(file)) dirs.push({ [value]: searchDirs(value, mask, deep) }) }
        else                                    { if (rxpFile.test(file)) files.push(value) }
	}
...
function PS(_opts)
{
    const opt = Object.assign({ exe: 'powershell.exe -NoProfile -Command ',  cwd: exports.path.ext,  time: 10000 },  _opts)

    this.run = (value, time = opt.time, exe = opt.exe) =>
    {
        const cmd = exe + value,   proc = child.spawn(cmd, [], {cwd: opt.cwd, shell: true})
