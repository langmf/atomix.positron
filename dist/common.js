...
function onDidChangeActiveTextEditor(editor)
{
    ...
    else                                    { updateVersion(doc);     statusVer.show() }
}
...
async function onDidOpenTextDocument(doc)
{
    ...
    if (doc.fileName.startsWith('Untitled-'))   untitledHeader(doc)
    else                                        PLG.load(PLG.file(doc.fileName))
...
function setActive()
{
    ...
    clearTimeout(setActive.tmr)
    exports.active = true
    setActive.tmr = setTimeout(() => exports.active = false,  tmo)
}
...
{
    const grp = vscode.window.tabGroups.activeTabGroup,   tab = grp.activeTab
    
    exports.isOpenFirst = true
    ...
    if (root.config.smartParentIncludes)
    {
        if (path.extname(tab.label).toLowerCase() === '.inc')
        {
            ...
            if (res.length)
            {
                ...
                cache.get(tab.input).wait = new Promise(resolve => {rsv = resolve})
                ...
                delete cache.get(tab.input).wait
                rsv()
            }
        }
    }
...
function openInclude(sel)
{
    ...
    const editor = sel.textEditor
    const inc    = getWordInclude(editor.document, sel.selections[0].active);                           if (!inc) return
    const file   = findInclude(inc.name)
...
{
    const tmo = root.config.output.DelayHide
    
    clearTimeout(OutputHide._tmr);          OutputHide._tmr = null
...
{
    const rxp = /include +"([^"]+)"/i,   range = doc.getWordRangeAtPosition(position, rxp)
...
function failRange(doc, position)
{
    ...
    for (const char of txt)     if (char === '"') q++
...
function getProc(doc, position)
{
    ...
    for (const item of Object.values(items))    if (item.range.contains(position)) return item
}
...
function parseSEM(doc, wr = false)
{
    const SEM = cache.get(doc).semantic;                if (!wr && SEM.$.items) return SEM.$.items

    const files = parseDoc(doc),   idoc = {},   idev = {},   words = []

    for (const file of Object.values(files))
    {
        for (const type of Object.values(file.types))   Object.assign(idoc, type.$items)
    }

    for (const sfr of getSFR(doc).filter(v => v.words))
    {
        Object.assign(idev, sfr.items);         words.push(sfr.words)
    }

    return SEM.items = { idoc,  idev,  words }
}
exports.parseSEM = parseSEM
...
function newTypes(value)
{
    ...
    const res = {
        _ : value,
        $(v) {
            return (v || "").split(",").reduce( (a,i) =>
                {
                    const x = this._[i.trim()];         (x && a.push(...x));          return a
                },
                []
            )
        }
...
function findInclude(name, value)
{
    ...
    if (value) {
        value = value.endsWith(path.sep) ? value : value + path.sep
        if (value !== arr[0]) arr.unshift(value)
    }
...
function listSymbols()
{
    ...
    for (const [k,v] of Object.entries(Enums))
    {
        ...
        const s = new vscode.DocumentSymbol(v.title, "", v.sym, r, r)
    ...
    return Object.entries(list).reduce( (r, [k,v]) =>
        {
            ...
            {
                if (showInRoot.includes(v.name.toLowerCase()))  return r.concat(v.children)
                r.push(v)
            }
            return r
        },
...
{
    const parseSpace = (v) => v.toLowerCase().replace(/^\w+[\t ]+/ig, '')

    const parseHelp = (v) => {
        const x = v.match(/^([-=*]){8}/)
        
        if (!x)     (help && help.push(v))
        else        help = (x[1] !== '-' && help) || []
    }
    ...
    const parseLast = (v) => {
        for (let q = 0, i = 0;  i < v.length;  i++)
        {
            ...
            if (v[i] === ':') {
                i = v.length - i;       rxp.lastIndex -= i;       m[5] = m[5].slice(0,-i);       return i
            }
        }
    }
    ...
    while ((m = rxp.exec(text)) !== null)
    {
        ...
        if      (m[6])   d = { name: m[6],    id: "label",                                          local: 1    }
        else if (m[7])   d = { name: m[7],    id: "end"                                                         }
        else if (m[8])   d = { name: m[8],    id: "include",                                        ofs: 1      }            
        else if (m[9])   d = { name: m[9],    id: "proc",                                           members: {} }
        else if (m[11])  d = { name: m[12],   id: parseSpace(m[11]),    last: parseLast(m[13]),     local: 1    }
        else if (m[14])  d = { name: m[15],   id: parseSpace(m[14]),    last: parseLast(m[16])                  }
        ...
        if (d.members)
        {
            ...
            {
                if (!v[3]) {
                    if (v.index === r.lastIndex) r.lastIndex++
                    continue
                }
                ...
                res.push({
                    local:1,   name:v[3],   code:v[0].substring(p),   id,   start,   end,   type,   token
                })
            }
        }
    }
...
async function parseSymbols(doc)
{
    ...
    const newSymbol = (item) =>
    {
        ...
        if (prc) {
            if (name in prc.members) return;        prc.members[name] = item
        } else {
            if (name in obj.$items)  return;        obj.$items[name]  = item
        }
        ...
        if (prc)    Blocks[Blocks.length - 1].children.push(sym)
        else        obj.children.push(sym)

        if (obj.kind === vscode.SymbolKind.Function || obj.kind === vscode.SymbolKind.Method)
        {
            Blocks.push(sym)
            Blocks.proc = item.members && item
        }
    }
    ...
    for (const x of getSymbols(doc.getText()))
    {
        if (x.id === "end")
        {
            ...
            if (b) (Blocks.proc || {}).range = b.range = new vscode.Range(b.range.start, doc.positionAt(x.end))
...
async function parseIncludes(doc, list, timeout = 5000)
{
    ...
    for (const v of arr)
    {
        ...
        await new Promise(resolve =>
        {
            ...
            iSym.then(() => {
                clearTimeout(tmo);          i.fsPath = iDoc.uri.fsPath;         resolve()
            })
        })
    }
...
function parseDevice(doc, list, old)
{
    ...
    if (devs.length) {
        dev = devs.pop();       name = dev.value.name;          if (dev.isLocal) { local = dev.value;    r = dev.value.range }
    }
    ...
    if (old.DEVICE.$.name === name) {
        list.DEVICE       = old.DEVICE.$
        list.DEVICE.range = list.DEVICE.selectionRange = r
        return
    } else {
...
{
    if (typeof doc === 'object') doc = doc.uri.fsPath
...
{
    const res = [],   keys = {},   esc = !/\n/.test(text),   styles = STS.getThemeStyle(keys),   items = parseSEM(doc)

    const dev = items?.idev,   wrd = !rep ? items?.idoc : Object.assign({}, items?.idoc, rep)
    ...
    if (mark) {
        if (!esc)   text = '<pre>' + text + '</pre>'
        else        text = cleanMark(text).replace(/\t/g, ()=>'&nbsp;'.repeat(6)).replace(/ {2,}/g, (m)=>'&nbsp;'.repeat(m.length))
    }
...
function setVersion(doc, prm)
{
    ...
    if (prm) {
        ...
        if (m[5]) {
                  v.Major    = parseInt(m[6])
                  v.Minor    = parseInt(m[7])
                  v.Release  = parseInt(m[8])
                  v.Build    = parseInt(m[9])
        }
    } else {
        if (v.Enable) {
            v.Build++
